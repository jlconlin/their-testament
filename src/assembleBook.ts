...
export interface AssembleBookResult {
  book: DocBook;
  diags: Diag[];
...
export async function assembleBook(
  ...
  opts: { title?: string; personName?: string | null; margins?: "fixed" | "mirrored" } = {},
): Promise<AssembleBookResult> {
  // ---- 1. classify every annotation --------------------------------------
  const scope = {
    scripture: new Map<string, Annotation[]>(),
    gc: [] as Annotation[],
