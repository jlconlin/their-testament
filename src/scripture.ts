...
export const HELP_COLLECTIONS: Record<string, string> = {
  tg: "Topical Guide",
  bd: "Bible Dictionary",
 ...
 * issue, then article.
 */
export const MAGAZINES: Record<string, string> = {
  ensign: "Ensign",
  liahona: "Liahona",
  "new-era": "New Era",
  "ya-weekly": "YA Weekly",
...
for (const slug of BOOK_ORDER.jst ?? []) {
  ...
  if (src) ABBREV[`jst/${slug}`] = `JST ${src}`;
}
...
export type Classification =
  ...
  | { scope: "manual"; manual: string; docPath: string; docUri: string }
