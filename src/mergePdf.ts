...
let pdfLibPromise: Promise<any> | undefined;
function loadPdfLib(): Promise<any> {
  return (pdfLibPromise ??= import(/* webpackIgnore: true */ PDF_LIB_URL));
}
...
function annotUri(lib: any, context: any, entry: any): string | null {
  const annot = entry instanceof lib.PDFRef ? context.lookup(entry) : entry;
  if (!(annot instanceof lib.PDFDict)) return null;
  let action = annot.get(lib.PDFName.of("A"));
  if (action instanceof lib.PDFRef) action = context.lookup(action);
  if (!(action instanceof lib.PDFDict)) return null;
  const uri = action.get(lib.PDFName.of("URI"));
  if (uri instanceof lib.PDFString || uri instanceof lib.PDFHexString) return uri.decodeText();
  return null;
}
 ...
 */
function readLinkTargets(lib: any, srcDoc: any, srcPageRefs: any[]): (number | null)[][] {
  return srcDoc.getPages().map((page: any) => {
    const annots = page.node.Annots();
    if (!annots) return [];
    const targets: (number | null)[] = [];
    for (let k = 0; k < annots.size(); k++) {
      let annot = annots.get(k);
      if (annot instanceof lib.PDFRef) annot = srcDoc.context.lookup(annot);
      targets.push(
        annot instanceof lib.PDFDict ? destPageIndexOf(lib, srcDoc.context, annot, srcPageRefs) : null,
      );
    }
    return targets;
  });
...
function remapLinks(lib: any, destDoc: any, copied: any[], targets: (number | null)[][]): void {
  const { PDFName } = lib;
  copied.forEach((page: any, i: number) => {
    const annots = page.node.Annots();
    if (!annots) return;
    const pageTargets = targets[i] ?? [];
    for (let k = 0; k < annots.size(); k++) {
      const target = pageTargets[k];
      if (target == null || !copied[target]) continue;
      let annot = annots.get(k);
      if (annot instanceof lib.PDFRef) annot = destDoc.context.lookup(annot);
      if (!(annot instanceof lib.PDFDict)) continue;
      annot.delete(PDFName.of("A"));
      annot.set(
        PDFName.of("Dest"),
        destDoc.context.obj([copied[target].ref, PDFName.of("XYZ"), null, null, null]),
      );
    }
  });
}
...
export async function readAnchors(bytes: Uint8Array): Promise<Map<string, number>> {
 ...
 */
function resolveCrossLinks(lib: any, destDoc: any): void {
  const { PDFName } = lib;
  const pages = destDoc.getPages();
  const defs = new Map<string, number>();
  const refs: { annots: any; slot: number; key: string }[] = [];
  ...
  pages.forEach((page: any, i: number) => {
    ...
    for (let k = 0; k < annots.size(); k++) {
      ...
      if (uri.startsWith(DEF_PREFIX)) {
        const key = uri.slice(DEF_PREFIX.length);
        if (!defs.has(key)) defs.set(key, i);
  ...
  for (const page of pages) {
    ...
    for (let k = 0; k < annots.size(); k++) {
      ...
      if (annotUri(lib, destDoc.context, entry)?.startsWith(DEF_PREFIX)) continue;
      keep.push(entry);
    }
    if (keep.length !== annots.size()) page.node.set(PDFName.of("Annots"), destDoc.context.obj(keep));
  }
}

function decodeTitle(lib: any, obj: any): string {
...
function destPageIndexOf(lib: any, context: any, itemDict: any, srcPageRefs: any[]): number | null {
  let destArray: any = null;
  const dest = itemDict.get(lib.PDFName.of("Dest"));
  if (dest instanceof lib.PDFArray) destArray = dest;
  else if (dest instanceof lib.PDFRef) {
    const resolved = context.lookup(dest);
    if (resolved instanceof lib.PDFArray) destArray = resolved;
  } else {
    ...
    if (actionDict instanceof lib.PDFDict) {
      ...
      if (d instanceof lib.PDFArray) destArray = d;
  ...
  if (!(pageRefOrObj instanceof lib.PDFRef)) return null;
  const idx = srcPageRefs.findIndex(
    (r) => r.tag === pageRefOrObj.tag && r.objectNumber === pageRefOrObj.objectNumber,
  );
  return idx >= 0 ? idx : null;
}

function walkOutline(lib: any, context: any, dict: any, srcPageRefs: any[]): OutlineItem[] {
...
function buildOutline(lib: any, destDoc: any, items: OutlineItem[], destPages: any[]): { firstRef: any; lastRef: any; count: number } {
...
export async function mergePdfs(pdfs: Uint8Array[]): Promise<Uint8Array> {
  ...
  for (const bytes of pdfs) {
    ...
    const srcPageRefs = srcDoc.getPages().map((p: any) => p.ref);
