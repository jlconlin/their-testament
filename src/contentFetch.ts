...
export interface RetryOpts {
  /** Attempts *after* the first. Default 3, i.e. up to 4 requests. */
  retries?: number;
  /** First backoff step; doubles each attempt. Default 600ms. */
  baseDelayMs?: number;
  /** Called before each wait, so a UI can say why it has paused. */
 ...
 */
export async function fetchContentJson<T>(url: string, opts: RetryOpts = {}): Promise<T> {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (!RETRYABLE.has(res.status)) throw new Error(`content ${res.status}`);
