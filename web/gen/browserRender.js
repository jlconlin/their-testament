...
import { mergePdfs, readAnchors, pageCount } from "./mergePdf.js";
...
async function newCompiler(fonts) {
...
async function compileOne(compiler, mainTypst, spec) {
    ...
    if (!result)
        throw new Error(`typst compile failed (${spec.mode}): ${JSON.stringify(diagnostics)}`);
    return result;
}
/** Entry point used by web/compile-worker.js -- one chunk, one fresh realm. */
export async function compileChunk(opts) {
...
function countUnits(part) {
    if (part.kind === "scripture") {
        return (part.chapters ?? []).reduce((n, c) => n + (c.verses?.length ?? 0), 0)
            + (part.documents ?? []).reduce((n, d) => n + (d.paragraphs?.length ?? 0), 0);
    }
    if (part.kind === "gc") {
...
class ProgressCounter {
    report;
    total;
    constructor(report, total = 1) {
        this.report = report;
        this.total = total;
    }
    done = 0;
    step(label) { this.report?.({ done: this.done, total: this.total, label }); }
    finishOne() { this.done++; }
    /** A bisected piece became two: the denominator has to grow with it. */
    addPieces(n) { this.total += n; }
}
/**
 ...
 */
function planPieces(part) {
    if (countUnits(part) <= PIECE_UNIT_CAP)
        return [part];
    const halves = splitPart(part);
    if (!halves)
        return [part]; // indivisible: let it try, and fail honestly
...
async function compilePart(ctx, part, out, prog) {
 ...
 */
export async function renderBookAuto(opts) {
