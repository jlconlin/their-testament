...
const LEGACY_FORMATS = ["gospel-library-preservation"];
// --- building --------------------------------------------------------------
...
export function wrapAnnotations(annotations, source = {}, exportedAt = new Date().toISOString()) {
    return {
...
function validateRecords(annotations, errors, warnings) {
    const seen = new Set();
    ...
    annotations.forEach((a, i) => {
        ...
        if ("highlights" in rec && rec.highlights != null && !Array.isArray(rec.highlights))
            badHighlights++;
        if ("tags" in rec && rec.tags != null && !Array.isArray(rec.tags))
            badTags++;
        if ("folders" in rec && rec.folders != null && !Array.isArray(rec.folders))
            badFolders++;
        for (const k of ["created", "lastUpdated"]) {
            const v = rec[k];
            if (typeof v === "string" && v !== "" && !ISO_RE.test(v))
                badDates++;
        }
    });
    if (missingId)
        errors.push(`${missingId} record(s) missing a string annotationId`);
    if (badHighlights)
        errors.push(`${badHighlights} record(s) have a non-array \`highlights\``);
    if (badTags)
        errors.push(`${badTags} record(s) have a non-array \`tags\``);
    if (badFolders)
        errors.push(`${badFolders} record(s) have a non-array \`folders\``);
    if (badDates)
        warnings.push(`${badDates} date field(s) are not ISO 8601`);
}
/**
...
export function validateEnvelope(raw) {
    ...
    if (Array.isArray(raw)) {
        warnings.push("input is a bare annotation array, not an envelope — wrapping as version 1; " +
            "prefer a real annotations.json envelope (see docs/annotations-format.md)");
        validateRecords(raw, errors, warnings);
        if (errors.length)
            return { ok: false, version: null, wrappedLegacy: true, errors, warnings, envelope: null };
        return {
            ok: true,
            version: ENVELOPE_VERSION,
            wrappedLegacy: true,
            errors,
            warnings,
            envelope: wrapAnnotations(raw, { exporter: "legacy-bare-array" }),
        };
    }
    // (3) not an object
    ...
    if (env.counts && typeof env.counts === "object" && !Array.isArray(env.counts)) {
        const declared = env.counts.total;
    ...
    if (errors.length)
        return { ok: false, version, wrappedLegacy: false, errors, warnings, envelope: null };
    const source = env.source && typeof env.source === "object" && !Array.isArray(env.source) ? env.source : {};
    return {
        ok: true,
        version,
        wrappedLegacy: false,
 ...
 */
export function readEnvelope(raw, onWarn = (m) => console.warn(`  ⚠ ${m}`)) {
