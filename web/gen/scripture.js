...
export function manualCategory(slug) {
    if (slug.startsWith("come-follow-me"))
    ...
    if (slug.includes("gospel-doctrine"))
        return "Handbooks and Guides";
    if (SEMINARY_RE.test(slug))
        return "Seminary and Institute";
    if (slug.startsWith("teachings-"))
        return "Teachings of Presidents of the Church";
    if (HANDBOOK_SLUGS.has(slug) || slug.startsWith("scouting-handbook"))
        return "Handbooks and Guides";
    if (FAITH_SLUGS.has(slug) || slug.startsWith("scripture-helps"))
        return "Faith and Doctrine";
    if (YOUTH_SLUGS.has(slug) || slug.startsWith("for-the-strength-of-youth") || slug.startsWith("my-foundation")
        || slug.startsWith("succeed-in-school"))
        return "For Youth and Families";
