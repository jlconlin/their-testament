 ...
 */
export function locate(text, startOffset, endOffset, leadingTokens = 0) {
    if (leadingTokens > 0) {
        if (startOffset > 0)
            startOffset = Math.max(0, startOffset - leadingTokens);
        if (endOffset > 0)
            endOffset = Math.max(0, endOffset - leadingTokens);
    }
